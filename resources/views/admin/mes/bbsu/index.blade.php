...
@section('breadcrumb')
    ...
    <strong>Battery breaking & separation unit</strong>
@endsection
...
@push('styles')
    <style>
        ...
        .fh-right svg {
            width: 16px;
            height: 16px;
            stroke: var(--gr-text-muted);
            transition: transform 0.3s;
        }

        .filter-card.open .fh-right svg {
        ...
        .search-input input:focus {
            border-color: var(--gr-primary);
            background: #fff;
            box-shadow: 0 0 0 3px var(--gr-primary-light);
        }
    ...
    <div class="page-wrapper">
        ...
        <div class="top-action-bar">
            <div class="top-action-left">
                ...
                <div class="page-info">
                    <h2>Battery Breaking & Separation Unit</h2>
                    <p>Manage Battery Breaking & Separation Unit, drafts and submissions</p>
                </div>
            </div>
            <div class="top-action-right">
                ...
                <a href="{{ route('admin.mes.bbsu.create') }}" class="btn-primary">
                    <svg viewBox="0 0 24 24" fill="none" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"
                        style="width:14px;height:14px;">
...
@push('scripts')
    <script>
                ...
                return `<tr>
                    ...
                    <td>
                        <div style="display:inline-flex;gap:6px">
                            <a href="${EDIT_BASE}${item.id}/edit" style="width:30px;height:30px;border-radius:7px;background:#f0fdf4;display:inline-flex;align-items:center;justify-content:center;color:#1e7b51;text-decoration:none;" title="Edit / View">
                            ...
                            ${isDraft ? `
                            <button style="width:30px;height:30px;border-radius:7px;border:none;background:#fff1f2;color:#ef4444;display:inline-flex;align-items:center;justify-content:center;cursor:pointer;" onclick="deleteReceiving(${item.id}, event)" title="Delete">
        ...
        async function updateSummaryCards(total) {
            ...
            const [dr, sr] = await Promise.all([apiFetch('/bbsu-batches?status=0&per_page=1'), apiFetch('/bbsu-batches?status=1&per_page=1')]);
        ...
        async function deleteReceiving(id, e) {
            ...
            const res = await apiFetch(`/bbsu-batches/${id}`, { method: 'DELETE' });
