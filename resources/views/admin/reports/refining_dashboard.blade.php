...
@push('scripts')
    ...
    <script>
        ...
        function openDrill(batchId) {
            ...
            // ── Raw Materials (no smelting batch column) ──
            ...
            // ── Chemicals & Metals (no smelting batch column) ──
        ...
        function exportDetailExcel() {
            ...
            if (r.raw_materials?.length) {
                const rmData = [['Material', 'Qty (KG)'],
                ...r.raw_materials.map(m => [m.material, m.qty])];
            ...
            if (r.chemicals?.length) {
                const chData = [['Chemical / Metal', 'Qty (KG)'],
                ...r.chemicals.map(c => [c.chemical, c.qty])];
                const ws5 = XLSX.utils.aoa_to_sheet(chData);
                ws5['!cols'] = [{ wch: 30 }, { wch: 14 }];
